  ...
  modal.addComponents(
    new ActionRowBuilder().addComponents(
      new TextInputBuilder()
        .setCustomId('pd_membro')
        .setLabel('Membro que solicitou (opcional)')
        .setPlaceholder('@Mencione ou deixe vazio')
...
async function handleModalPd(interaction) {
  ...
  const membro = interaction.fields.getTextInputValue('pd_membro').trim() || null;
  const id     = interaction.fields.getTextInputValue('pd_id').trim();
  const quem   = interaction.fields.getTextInputValue('pd_quem').trim();
...
module.exports = { handlePdChannel, handlePdBotao, handleModalPd };
