...
const TIPOS = [
  ...
  { value: 'farm',     label: 'Farm',      emoji: '🌾', descricao: 'Assuntos relacionados ao farm'   },
];
  ...
  try {
    ...
    const jaExiste  = mensagens.some(
      (m) => m.author.id === client.user.id && (
        (m.embeds.length > 0 && m.components.length > 0 && m.embeds[0]?.title?.includes('Ticket')) ||
...
async function handleTicketSelect(interaction) {
  ...
  try {
    ...
    for (const row of botoesStaff(member.id)) {
      container.addActionRowComponents(row);
    }

    await canal.send({ components: [container], flags: MessageFlags.IsComponentsV2 });
    await interaction.editReply({ content: `✅ Ticket aberto! ${canal}` });
  } catch (err) {
    console.error(`[${new Date().toISOString()}] Erro ao criar ticket:`, err);
    await interaction.editReply({ content: '❌ Erro ao criar o ticket. Verifique se o bot tem permissão de **Gerenciar Canais**.' });
  }
}
...
async function handleTicketNotifModal(interaction, userId) {
  ...
  try {
    ...
    await membro.send(dmEmbed('📢 Mensagem do Ticket',
      `🎫 **Ticket:** \`${interaction.channel.name}\`\n\n${mensagem}\n\n-# Never Pure — Central de Atendimento`,
      0x5865F2));
...
async function handleTicketSalvar(interaction, userId) {
  ...
  try {
    ...
    if (criador) {
      ...
      await criador.send({
        ...dmEmbed('💾 Ticket Salvo',
          `🎫 **Ticket:** \`${interaction.channel.name}\`\n💾 **Salvo por:** ${interaction.user.tag}\n📎 Transcript em anexo.\n\n-# Never Pure — Central de Atendimento`,
          0x3498DB),
        files: [arquivoDM],
      }).catch(() => {});
    }
